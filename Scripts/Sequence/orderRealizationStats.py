...
def make_orderFulfill_dict(model):
    ...
    for i in model.riders_set:
        if model.sol[i] >= 0.5 and model.setting['match_time'] + model.time_dict[i] <= model.rider_dict[i][0] + model.rider_dict[i][1]:
        #if model.sol[i] >= 0.5 and model.time_dict[i] <= model.rider_dict[i][1]:
...
def make_performance_stats(model):
    ...
    return {'ID':model.ID,'avg_pickup_time': avg_pickup_time, 'avg_pickup_distance': avg_pickup_distance, 
            ...
            'order_fulfill_rate': order_fulfill_rate}
