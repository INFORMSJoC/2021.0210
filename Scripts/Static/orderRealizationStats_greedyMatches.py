# -*- coding: utf-8 -*-
"""
Created on Wed Sep  7 16:00:03 2022

@author: Rong Dongling
"""

# calculate the pick-up time for all rider driver assignments
def make_pickupTime_vector_2(distance_matrix, speeds, sol_vec):
...
def make_pickupDistance_vector_2(distance_matrix, sol_vec):
    N_riders = len(sol_vec)
...
def make_orderFulfill_vector_2(riders_wait, time_vec):
...
def make_performance_stats(model):
    ...
    time_vec = make_pickupTime_vector_2(model.distances, model.drivers_speed, model.sol)
    distance_vec = make_pickupDistance_vector_2(model.distances, model.sol)
    orderFulfill_vec = make_orderFulfill_vector_2(model.riders_wait, time_vec)
